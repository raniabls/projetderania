<template>
  <div id="app">
    ...
    <div class="container">
      <!-- v-on (@) pour écouter l'événement submit et la méthode addTask sera appelée lors on fait le submit 
        du formulaire, prevent évite le comportement par défaut de soumission la page ne sera pas rechargée-->
      <form @submit.prevent="addTask" id="taskform">
        <h1> Tâches notes</h1>
        <div class="itemswrap">
          <label for="tasktitle">Titre: </label> 
          <!-- on a utilise la directive v-model pour lier le champ de saisie au modèle Vue (taskForm.titre) 
           toute modification dans le champ de saisie sera reflétée dans la propriété taskForm.titre et vice vers -->
      ...
      <div id="tasklist">
        ...
        <!--on a creer une boucle pour chaque tâche dans la liste en utilisant l'index comme clé unique pour 
          chaque élément dans la listes et il sera représenté par tous ca et onapplique a la tache la couleur on a saisit -->
        <div v-for="(task, index) in tasks" :key="index" class="task" :style="{ backgroundColor: task.color }">
          <div class="task-content">
           ...
           <!-- Si la tâche n'est pas en mode édition on affiche etat et bouton editer etat-->
            ...
            <!--Si la tâche a des détails (qui sont string) et n'est pas en mode édition affiche bouton modifier les détails sous forme de checkbox-->
              <div v-if="task.details && typeof task.details === 'string' && !task.editingDetails">
               ...
               split('/n') est utilisée pour diviser les détails en un tableau de lignes en fonction des sauts de ligne ('\n')
           ...
           <!--Si les détails sont en mode édition affiche affiche textarea pour modifer et bouton enregistrer-->
...
export default {
  ...
  // Json.parse: La valeur récupérée à partir du localStorage est généralement un string 
  //elle est utilisé pour convertir cette chaîne JSON en un objet JavaScript pour pouvoir la manipuler
  // []:Si aucune tâche n'est stockée ou si la conversion échoue this.tasks sera initialisé avec un tableau vide []
  },

  methods: {
    addTask() {
    // on cree un nouveau objet newTask représentant une nouvelle tâche en utilisant les valeurs acteuls du formulaire
      const newTask = {
        ...
        editingPriority: false, // utilisée pour suivre si letat de la tache est en cours d'édition
        editedPriority: '',
      };
